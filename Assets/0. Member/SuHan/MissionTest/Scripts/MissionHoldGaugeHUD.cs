...
/// <summary>
/// [테스트 전용] 미션 오브젝트에서 F를 누르고 있는 동안 화면 아래 가운데에 진행 게이지(채워지는 바 + %)를 띄운다.
///  에셋 없이 IMGUI 로 그린다. 옛 발전기/안테나 미션의 게이지(채워지는 바 + 퍼센트)와 같은 형태.
///
/// [읽는 값] MissionInteractable 의 공개 값만 쓴다.
///  - IsUsedByLocalPlayer : 지금 내가 이 오브젝트를 쓰고 있는가 (호스트가 허용한 경우에만 true)
///  - HoldProgress01      : 홀드 진행률 0~1 (호스트가 계산해서 동기화한 값)
///  그래서 게이지가 떴다 = 호스트가 상호작용을 허용했다는 뜻이다. F를 눌러도 게이지가 안 뜨면 거부된 것이다.
///  정식 UI 를 만들 때도 같은 두 값을 읽으면 된다.
/// </summary>
public class MissionHoldGaugeHUD : MonoBehaviour
{
    ...
    private MissionInteractable[] interactables = new MissionInteractable[0];
    private float nextSearchTime;
    private GUIStyle textStyle;

    private void OnGUI()
    {
        MissionInteractable active = FindActive();
    ...
    /// <summary>내가 지금 사용 중인 미션 오브젝트. 오브젝트 목록은 1초마다 다시 찾는다 (스폰이 늦게 끝날 수 있어서).</summary>
    private MissionInteractable FindActive()
    {
        if (Time.unscaledTime >= nextSearchTime)
        {
            interactables = FindObjectsByType<MissionInteractable>(FindObjectsSortMode.None);
            nextSearchTime = Time.unscaledTime + 1f;
        }

        foreach (MissionInteractable interactable in interactables)
        {
            if (interactable == null || interactable.Object == null || !interactable.Object.IsValid)
                continue;

            if (!interactable.Consumed && interactable.IsUsedByLocalPlayer)
                return interactable;
        }
